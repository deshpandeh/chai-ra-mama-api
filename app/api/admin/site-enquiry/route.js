...
import SiteEnquiry from "@/models/siteEnquiry/SiteEnquiry";
...
export async function GET(request, response) {
    try {
        await connectToDb();
        const data = await SiteEnquiry.find({updateStatus:{$nin:["linked","unlinked"]}})
        ...
        return NextResponse.json({
            message: "Error in getting SiteEnquiry Details" + e
        }, { status: 500 })
    }
}
export async function POST(request, response) {
    try {
        ...
        await connectToDb();
        await SiteEnquiry.create({contactName,contactEmail,contactSubject,contactMessage})
        return NextResponse.json({
...
export async function PATCH(request) {
    try {
        const body = await request.json();
        const {_id,authId,contactName,contactEmail,contactPhone}= body
        await connectToDb();
        const isAdminExist = await AdminSignUp.findOne({ email: authId?.id })
        if (isAdminExist) {
            const isProductExist = await SiteEnquiry.findOne({ _id })
            ...
            if (isProductExist && isProductExist?.updateStatus == "none") {
                const newData=await SiteEnquiry.create({contactName,contactEmail,contactPhone, updateStatus: "linked", productId: _id })
                ...
                await SiteEnquiry.findByIdAndUpdate(_id, { updateStatus: "requested" })
                await ActionAuth.create({ clientRef: clientReference, adminRef: adminReference, productId: newData?._id, request: "updated", end_url: "siteenquiries" })
...
export async function DELETE(request) {
    try {
        ...
        if (isAdminExist) {
            ...
            await SiteEnquiry.findByIdAndUpdate(_id, { deleteStatus: "requested" })
            await ActionAuth.create({clientRef:clientReference,adminRef:adminReference,productId:_id,request:"delete",end_url:"siteenquiries"})
