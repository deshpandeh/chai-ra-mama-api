...
export async function GET(request, response) {
    try {
        await connectToDb();
        const data = await StoreLocator.find({updateStatus:{$nin:["linked","unlinked"]}})
        data.sort((a,b)=>a.storeCity.localeCompare(b.storeCity))
        ...
        return NextResponse.json({
            message: "Error in getting StoreLocator Details" + e
        }, { status: 500 })
    }
}
export async function POST(request, response) {
    try {
        ...
        const {storeCity}= body
        await connectToDb();
        await StoreLocator.create({storeCity})
        return NextResponse.json({
            message: "StoreLocator Details added successfully",
        }, { status: 200 })
    }
    catch (e) {
        return NextResponse.json({
...
export async function PATCH(request) {
    try {
        const body = await request.json();
        const {_id, authId, storeCity}= body
        await connectToDb();
        const isAdminExist = await AdminSignUp.findOne({ email: authId?.id })
        if (isAdminExist) {
            const isProductExist = await StoreLocator.findOne({ _id })
            ...
            if (isProductExist && isProductExist?.updateStatus == "none") {
                const newData=await StoreLocator.create({ storeCity, updateStatus: "linked", productId: _id })
                ...
                await StoreLocator.findByIdAndUpdate(_id, { updateStatus: "requested" })
                await ActionAuth.create({ clientRef: clientReference, adminRef: adminReference, productId: newData?._id, request: "updated", end_url: "storelocators" })
...
export async function DELETE(request) {
    try {
        ...
        if (isAdminExist) {
            ...
            await StoreLocator.findByIdAndUpdate(_id, { deleteStatus: "requested" })
            await ActionAuth.create({clientRef:clientReference,adminRef:adminReference,productId:_id,request:"delete",end_url:"storelocators"})
