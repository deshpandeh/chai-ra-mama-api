...
export async function GET(request, response) {
    try {
        ...
        await connectToDb();
        const data = await SubMenu.find({parentId:_id,updateStatus:{$nin:["linked","unlinked"]}})
...
export async function POST(request, response) {
    try {
        ...
        const {parentId,itemName,itemImage,itemPrice}= body
        await connectToDb();
        await SubMenu.create({parentId,itemName,itemImage,itemPrice})
        return NextResponse.json({
            message: "SubMenu Details added successfully",
        }, { status: 200 })
    }
    catch (e) {
        return NextResponse.json({
...
export async function PATCH(request) {
    try {
        const body = await request.json();
        const {_id, authId, itemName,itemImage,itemPrice}= body
        await connectToDb();
        const isAdminExist = await AdminSignUp.findOne({ email: authId?.id })
        if (isAdminExist) {
            const isProductExist = await SubMenu.findOne({ _id })
            ...
            if (isProductExist && isProductExist?.updateStatus == "none") {
                const newData=await SubMenu.create({ itemName,itemImage,itemPrice, updateStatus: "linked", productId: _id })
                ...
                await SubMenu.findByIdAndUpdate(_id, { updateStatus: "requested" })
                await ActionAuth.create({ clientRef: clientReference, adminRef: adminReference, productId: newData?._id, request: "updated", end_url: "submenus" })
...
export async function DELETE(request) {
    try {
        ...
        if (isAdminExist) {
            ...
            await SubMenu.findByIdAndUpdate(_id, { deleteStatus: "requested" })
            await ActionAuth.create({clientRef:clientReference,adminRef:adminReference,productId:_id,request:"delete",end_url:"submenus"})
